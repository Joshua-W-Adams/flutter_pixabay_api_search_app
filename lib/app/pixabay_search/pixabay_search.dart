...
import 'package:flutter_pixabay_api_search_app/widgets/pixabay_network_image.dart';
      ...
      builder: (_, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (state.query == '') {
            ...
            if (state.results.hits.length < 1) {
            ...
            return ImageListView(hits: state.results.hits);
          }
        }
      },
    );
  ...
  Widget build(BuildContext context) {
    return CupertinoScrollbar(
      child: ListView.builder(
        ...
        itemBuilder: (_, index) {
          PixabayMediaModel hit = hits[index];
          return Padding(
            padding: EdgeInsets.all(8.0),
            child: PixabayNetworkImage(
